...
class TestMatchResumesToResumeMatcherServices:
    """
    Test TestMatchResumesToResumeMatcherServices
    """
    ...
    def create_payload(self, param=''):
        ...
        fake = Faker()
        if param == 'resume_id':
            resume_id = "SIMP-R-{}-HIRE-THERMOFISHER-{}".format(fake.random_int(1500000000, 1583916379),
                                                                fake.random_int())
        else:
            resume_id = 'SIMP-R-1596184855-HIRE-THERMOFISHER-5324'
    ...
    def test_successful_for_match_resumes_to_resume(self, test_app):
        """
        Test successful Match Resume To Resume
        :param test_app:
        :return:
        """
        logger.info("************************************* Start of Match Resumes to Resume test case***************************************")
        ...
        if response_result.get("code") == HTTP_200_OK or \
            ...
            candidate_resume = test_app.post('/match/resumes/to/resume',
                                             headers=headers,
            ...
            if candidate_resume.status_code == HTTP_200_OK:
                ...
                if response_result.get("code") == HTTP_200_OK or \
                    ...
                    logger.info("***********************************Pass Test Case for Matching Resumes to Resume ****************************")
            ...
            assert response_result.get("code") == HTTP_404_NOT_FOUND
            logger.info("***********************************Fail Test Case for Matching Resumes to resume ****************************")
            logger.info("Code we expect is 404, code we receive is:{}".format(response_result.get("code")))

    def test_match_resumes_fails_for_false_resume_id(self, test_app):
        """
        Test successful Match Resume To Resume
        :param test_app:
        :return:
        ...
        false_id = "SIMP-R-{}-HIRE-THERMOFISHER-{}".format(fake.random_int(1500000000, 1583916379),
                                                           fake.random_int())
        response_register = test_app.post('/match/resumes/to/resume', headers=headers,
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            if int(response_result.get('code')) == HTTP_404_NOT_FOUND:
                assert response_result.get("code") == HTTP_404_NOT_FOUND
                logger.info("***********************************Matching Resume to Resume Fails For Wrong Resume Id ****************************")
                logger.info("Code we expect is 404, code we receive is:{}".format(response_result.get("code")))

    def test_match_resumes_fails_for_resume_id(self, test_app):
        """
        Test failure for missing of resume id
        :param test_app:
        :return:
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching resumes Fails For Absent Of Resume Id*******")
            ...
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Resume Id****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_no_of_matches(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Number Of Matches****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_refresh_rate(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Refresh Rate****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_titles(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Titles****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_skills(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Skills*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_skills_must_all_exist(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Skills Must All Exist*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get('error') == 'skills_must_all_exist is not provided'
            logger.info("***************************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Skilla Must All Exist****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_employers(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Employers*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_employers_must_all_be_current_employer(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            response_result = response_register.json()
            logger.info(
                "*******Matching Resume to Resume Fails For Absent Of Employers Must All Be Current Employer*******")
            if not response_result.get('detail').get('data'):
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Employers Must All Be Current Employer****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_date_range_max(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Date Range Max*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get('error') == 'date_range_max is not provided'
            logger.info("***************************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of For Absent Of Date Range Max****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_date_range_min(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Date Range Min*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_school_name(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of School Name*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_degree_major(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Degree Major*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_degree_type(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Degree Type*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_degree_name(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Degree Name****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_minimum_gpa(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Minimum GPA*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_country_code(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Country Code*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_region(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Region*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_municipality(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Municipality*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_postal_code(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Postal Code*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_latitude(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Latitude*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_longitude(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Longitude*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_distance(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Distance*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_distance_unit(self, test_app):
        """
        Test failure for distance
        :param test_app:
        :return:
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Distance Unit*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_geocode_provider(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Geocode Provider*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_geocode_provider_key(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Geocode Provider Key*******")
    ...
    def test_match_resumes_to_resume_fails_for_search_expression(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Search Expression*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_languages_known(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Languages Known*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_languages_known_must_all_exist(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Languages Known Must All Exist*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_current_management_level(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Current Management Level*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_document_languages(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Document Languages*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_months_exp_min(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Months Of Minimum Experience*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_months_exp_max(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Months Of Maximum Experience*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_months_manage_exp_min(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            response_result = response_register.json()
            logger.info(
                "*******Matching Resume to Resume Fails For Absent Of Months Of Minimum Management Experience*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get('error') == 'months_manage_exp_min is not provided'
            logger.info("***************************************************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Months Of Minimum Management Experience****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_months_manage_exp_max(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            response_result = response_register.json()
            logger.info(
                "*******Matching Resume to Resume Fails For Absent Of Months Of Maximum Management Experience*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get('error') == 'months_manage_exp_max is not provided'
            logger.info("***************************************************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Months Of Maximum Management Experience****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_resumes_to_resume_fails_for_exec_type(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Exec Type*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_resumes_to_resume_fails_for_certifications(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Resume to Resume Fails For Absent Of Certifications*******")
            if not response_result.get('detail').get('data'):
            ...
            logger.info("***********************************Matching Resume to Resume Fails For Absent Of Certifications****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))
        logger.info("************************************* End of Match Resumes to Resume test case***************************************")