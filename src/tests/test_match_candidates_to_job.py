...
from src.tests.list_jobs_for_test import ListJobsForTest
from starlette.status import HTTP_200_OK, HTTP_404_NOT_FOUND, HTTP_204_NO_CONTENT, HTTP_422_UNPROCESSABLE_ENTITY, HTTP_400_BAD_REQUEST
...
class TestMatchCandidatesToJobMatcherServices:
    """
    Test TestMatchCandidatesToJobMatcherServices
    """
    ...
    def create_success_payload(self, job_id):
        """
        Create Payload
        :param param: String
        :return:
    ...
    def create_payload(self, param=''):
        ...
        if param == 'job_id':
            ...
            list_jobs = ListJobsForTest()
            source_id = list_jobs.get_job_sources()
            source_jobs = {}
            if source_id.get("source_id"):
                source_jobs = list_jobs.get_job_ref_num(source_id.get("source_id"))
            if source_jobs.get("jobs_list"):
                job_id = random.choice(source_jobs)
            else:
    ...
    def test_successful_for_match_candidates_to_job(self, test_app):
        """
        Test successful Match Candidates To Job
        :param test_app:
        :return:
        """
        logger.info("************************************* Start of Match Candidates to Job test case***************************************")
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            job_data = test_app.post('/match/candidates/to/job',
                                     headers=headers,
                                     data=self.create_success_payload(latest_test_job))
            if job_data.status_code == HTTP_200_OK:
                # Try to assert the token value received
                response_result = job_data.json()
                assert response_result.get("code") == HTTP_200_OK or \
                       response_result.get("code") == HTTP_204_NO_CONTENT

                logger.info("***********************************Pass test case for Match Candidates to Job****************************")
    ...
    def test_match_candidates_to_job_fails_for_job_id(self, test_app):
        """
        Test failure for missing of job id
        :param test_app:
        :return:
        """
        ...
        authorization_response = fake_registration.register_client(test_app)
        headers = fake_registration.request_headers(authorization_response.get('client_id'))
        response_register = test_app.post('/match/candidates/to/job', headers=headers,
                                          data=self.create_payload('job_id'))
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Jobs Fails For Absent Of Job Id*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get(
                    'error') == 'job_id is not provided'
            logger.info("***************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Job Id****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_no_of_matches(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Number Of Matches****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_refresh_rate(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Refresh Rate****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_titles(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Titles****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_skills(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Skills*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_skills_must_all_exist(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Skills Must All Exist*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get('error') == 'skills_must_all_exist is not provided'
            logger.info("***************************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Skilla Must All Exist****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_employers(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Employers*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_employers_must_all_be_current_employer(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            response_result = response_register.json()
            logger.info(
                "*******Matching Candidates To Job Fails For Absent Of Employers Must All Be Current Employer*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get(
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Employers Must All Be Current Employer****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_date_range_max(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Date Range Max*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get('error') == 'date_range_max is not provided'
            logger.info("***************************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of For Absent Of Date Range Max****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_date_range_min(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Date Range Min*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_school_name(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of School Name*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_degree_major(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Degree Major*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_degree_type(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Degree Type*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_degree_name(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Degree Name****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_minimum_gpa(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates to Job Fails For Absent Of Minimum GPA*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_country_code(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Country Code*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_region(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Region*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_municipality(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Municipality*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_postal_code(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Postal Code*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_latitude(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Latitude*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_longitude(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Longitude*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_distance(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Distance*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_distance_unit(self, test_app):
        """
        Test failure for distance
        :param test_app:
        :return:
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Distance Unit*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_geocode_provider(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Geocode Provider*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_geocode_provider_key(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Geocode Provider Key*******")
    ...
    def test_match_candidates_to_job_fails_for_search_expression(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Search Expression*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_languages_known(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Languages Known*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_languages_known_must_all_exist(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Languages Known Must All Exist*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_current_management_level(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Current Management Level*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_document_languages(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Document Languages*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_months_exp_min(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Months Of Minimum Experience*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_months_exp_max(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Months Of Maximum Experience*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_months_manage_exp_min(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            response_result = response_register.json()
            logger.info(
                "*******Matching Candidates To Job Fails For Absent Of Months Of Minimum Management Experience*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get('error') == 'months_manage_exp_min is not provided'
            logger.info("***************************************************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Months Of Minimum Management Experience****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_months_manage_exp_max(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            ...
            response_result = response_register.json()
            logger.info(
                "*******Matching Candidates To Job Fails For Absent Of Months Of Maximum Management Experience*******")
            if not response_result.get('detail').get('data'):
                assert response_result.get('detail').get('error') == 'months_manage_exp_max is not provided'
            logger.info("***************************************************************************************")
        else:
            assert response_register.status_code == HTTP_422_UNPROCESSABLE_ENTITY
            logger.info("***********************************Matching Candidates to Job Fails For Absent Of Months Of Maximum Management Experience****************************")
            logger.info("Code we expect is 422, code we receive is:{}".format(response_register.status_code))

    def test_match_candidates_to_job_fails_for_exec_type(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Exec Type*******")
            if not response_result.get('detail').get('data'):
    ...
    def test_match_candidates_to_job_fails_for_certifications(self, test_app):
        """
        ...
        if response_register.status_code == HTTP_200_OK:
            # Try to assert the token value received
            response_result = response_register.json()
            logger.info("*******Matching Candidates To Job Fails For Absent Of Certifications*******")
            if not response_result.get('detail').get('data'):
        ...
        logger.info("************************************* End of Match Candidates to Job test case***************************************")